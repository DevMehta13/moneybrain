...
interface TransactionDao {
    ...
    @Query("UPDATE transactions SET tripId = NULL WHERE tripId = :tripId") suspend fun clearTrip(tripId: Long): Int
...
@Dao interface TripDao { @Query("SELECT * FROM trips ORDER BY startedAt DESC") fun observeAll(): Flow<List<TripEntity>>; @Insert suspend fun insert(item: TripEntity): Long; @Query("SELECT * FROM trips WHERE id = :id") suspend fun getById(id: Long): TripEntity?; @Query("UPDATE trips SET endedAt = :at WHERE id = :id") suspend fun stop(id: Long, at: Long): Int; @Query("DELETE FROM trips WHERE id = :id") suspend fun deleteById(id: Long): Int; @Query("SELECT * FROM trips WHERE endedAt IS NULL AND startedAt <= :at ORDER BY startedAt DESC LIMIT 1") suspend fun activeAt(at: Long): TripEntity? }
